-- ============================================================================
-- 0017  Advance (partial) payment
-- ============================================================================
-- The customer may pay a slice now and the rest to the courier. The slice is
-- a percentage of the SUBTOTAL plus the WHOLE delivery charge -- we pay the
-- courier in full regardless of what the customer has settled -- with a floor
-- so the advance is worth collecting.
--
-- Both figures are computed inside place_order from a settings row. The client
-- may render them; it may never submit them.
--
-- Idempotent.

alter table public.orders
  add column if not exists advance_paisa integer not null default 0,
  add column if not exists due_on_delivery_paisa integer not null default 0;

-- Existing orders were all paid under the old all-or-nothing rule. Backfill
-- before adding the constraint, or every historical row fails it.
update public.orders
   set due_on_delivery_paisa = total_paisa - advance_paisa
 where advance_paisa + due_on_delivery_paisa <> total_paisa;
...
alter table public.orders
  add constraint orders_advance_split check (
    ...
    and advance_paisa + due_on_delivery_paisa = total_paisa
  );

insert into public.settings (key, value, description, is_public) values
  ('advance_payment',
   '{"enabled": true, "percent": 10, "min_paisa": 20000}'::jsonb,
   'Partial payment: percent of subtotal (plus the full delivery charge) collected up front, and the smallest advance worth taking, in paisa.',
   true)
on conflict (key) do nothing;

-- Re-created to compute and store the split.
...
begin
  ...
  then
    ...
    v_advance := least(v_advance, v_total);
    v_due := v_total - v_advance;

  elsif p_payment_method = 'cod' then
    -- Nothing up front; the courier collects everything.
    v_advance := 0;
    v_due := v_total;
  else
  ...
  insert into public.orders (
    ...
    advance_paisa, due_on_delivery_paisa,
  ...
  values (
    ...
    case when v_advance > 0 then v_advance else v_order.total_paisa end
  );
